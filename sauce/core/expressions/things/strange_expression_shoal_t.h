...
public:
	...
	{
		shoal.update(sym("strange::expression_shoal::create"), native_function::create(&expression_shoal_t<>::create__));
	}

	// function
	inline any_a<> operate(any_a<>& thing, list_a<> const& list) const
	{
		return unordered_shoal_t<>::create__(list_operator::create(_terms, thing, list));
	}
