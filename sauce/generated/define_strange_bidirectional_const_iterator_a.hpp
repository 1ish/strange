...
inline bool check(bidirectional_const_iterator_a<_1__chk> const& value) noexcept;
...
protected:
	...
	{
		return *std::static_pointer_cast<___bidirectional_const_iterator_a_handle_base___ const>(handle_);
	}
	...
	inline ___bidirectional_const_iterator_a_handle_base___& write() noexcept
	{
		...
		return *std::static_pointer_cast<___bidirectional_const_iterator_a_handle_base___>(handle_);
	}

private:
	template <typename ___TTT___, typename _1__chk>
	friend inline bool check(bidirectional_const_iterator_a<_1__chk> const& value) noexcept;
...
public:
	...
	{
		if (handle && !std::dynamic_pointer_cast<___bidirectional_const_iterator_a_handle_base___>(handle))
		{
			throw dis("bidirectional_const_iterator_a constructor failed to cast from base to bidirectional_const_iterator_a");
		}
	}
#else
	template <typename ___TTT___>
...
#ifdef STRANGE_CHECK_STATIC_CASTS
	template <typename ___TTT___>
	explicit inline bidirectional_const_iterator_a(std::shared_ptr<___TTT___>& handle, ___reference_tag___)
	...
	inline bidirectional_const_iterator_a& operator=(std::shared_ptr<___TTT___> const& handle)
	{
		if (handle && !std::dynamic_pointer_cast<___bidirectional_const_iterator_a_handle_base___>(handle))
		{
			throw dis("bidirectional_const_iterator_a assignment failed to cast from base to bidirectional_const_iterator_a");
		}
		handle_ = handle;
		return *this;
	}
...
{
	auto& shoal = shared();
	reflection<bidirectional_const_iterator_a<_1_>>::share(shoal);
	return shoal;
}();
