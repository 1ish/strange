...
private:
	...
	friend inline bool check(forward_iterator_a<_1__chk> const& value) noexcept;

public:
	static inline bool ___check___(___SHARED___ const& handle) noexcept
	{
		return bool(std::dynamic_pointer_cast<___forward_iterator_a_handle_base___>(handle));
	}
	...
	static inline forward_iterator_a ___cast_ref___(any_a<> const& thing)
	{
		auto const ptr = std::dynamic_pointer_cast<___forward_iterator_a_handle_base___>(thing.___handle___);
		if (ptr)
		{
			return forward_iterator_a{ ptr, ___reference_tag___{} };
		}
		return forward_iterator_a{ forward_iterator_d<_1_>{ thing, ___reference_tag___{} } };
	}

	static inline forward_iterator_a ___cast_dup___(any_a<> const& thing)
	{
		auto const ptr = std::dynamic_pointer_cast<___forward_iterator_a_handle_base___>(thing.___handle___);
		if (ptr)
		{
			return forward_iterator_a{ ptr, ___duplicate_tag___{} };
		}
		return forward_iterator_a{ forward_iterator_d<_1_>{ thing, ___duplicate_tag___{} } };
	}
	...
	explicit inline forward_iterator_a(std::shared_ptr<___TTT___> const& handle)
		: any_a{ handle }
	{
		if (handle && !std::dynamic_pointer_cast<___forward_iterator_a_handle_base___>(handle))
		{
			throw dis("forward_iterator_a constructor failed to cast from base to forward_iterator_a");
		}
	}
#else
	template <typename ___TTT___>
