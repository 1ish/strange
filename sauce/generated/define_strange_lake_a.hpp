...
inline bool check(lake_a<_1_> const& value) noexcept;
...
private:
	...
	struct ___lake_a_handle_final___ final : ___lake_a_handle___<___TTT___>
	{
		...
		{
			return std::make_shared<___lake_a_handle_final___>(___lake_a_handle___<___TTT___>::value_);
		}
	};
	...
	inline ___lake_a_handle_base___ const& read() const noexcept
	{
		return *std::static_pointer_cast<___lake_a_handle_base___ const>(handle_);
	}

	inline ___lake_a_handle_base___& write() noexcept
	{
		if (!handle_.unique())
		{
		...
		return *std::static_pointer_cast<___lake_a_handle_base___>(handle_);
	}

	template <typename ___TTT___, typename _1_>
	friend inline bool check(lake_a<_1_> const& value) noexcept;
...
public:
	...
	template <typename ___TTT___>
	explicit inline lake_a(std::shared_ptr<___TTT___> const& handle)
		: inventory_a{ handle }
	{
		if (handle && !std::dynamic_pointer_cast<___lake_a_handle_base___>(handle))
		{
			throw dis("lake_a constructor failed to cast from base to lake_a");
		}
	}
#else
	template <typename ___TTT___>
	...
	inline lake_a& operator=(std::shared_ptr<___TTT___> const& handle)
	{
		if (handle && !std::dynamic_pointer_cast<___lake_a_handle_base___>(handle))
		{
			throw dis("lake_a assignment failed to cast from base to lake_a");
		}
		handle_ = handle;
		return *this;
	}
