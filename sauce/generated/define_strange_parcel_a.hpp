...
private:
	...
	struct ___parcel_a_handle_final___ final : ___parcel_a_handle___<___TTT___>
	{
		...
		{
			return std_make_shared<___parcel_a_handle_final___>(___parcel_a_handle___<___TTT___>::___value___);
		}
	};
...
public:
	...
	template <typename ___TTT___>
	explicit inline parcel_a(std_shared_ptr<___TTT___> const& handle) noexcept
	...
	template <typename ___TTT___>
	explicit inline parcel_a(std_shared_ptr<___TTT___>& handle, any_a<>::___reference_tag___) noexcept
	...
	explicit inline parcel_a(___TTT___ value) noexcept
		: collection_a< any_a<> , parcel_a<> , parcel_a<> >{ std_make_shared<___parcel_a_handle_final___<typename std::remove_reference_t<___TTT___>>>(std::move(value)) }
	{
		any_a<>::___handle___->___weak___(any_a<>::___handle___);
	}
	...
	explicit inline parcel_a(any_a<>::___variadic_tag___, ___TTT___*, Args&&... args)
		: collection_a< any_a<> , parcel_a<> , parcel_a<> >{ std_make_shared<___parcel_a_handle_final___<typename std::remove_reference_t<___TTT___>>>(any_a<>::___variadic_tag___{}, std::forward<Args>(args)...) }
	{
		any_a<>::___handle___->___weak___(any_a<>::___handle___);
	}

	template <typename ___TTT___>
	inline parcel_a& operator=(std_shared_ptr<___TTT___> const& handle) noexcept
	{
		assert(!handle || std::dynamic_pointer_cast<___parcel_a_handle_base___>(handle));
...
public:
	inline any_a<> release__(list_a<> const& arguments) const
	{
		...
		if (!op)
		{
			throw dis("dynamic parcel_d::release_ passed non-existent member");
		}
	...
	inline any_a<> release_unique__(list_a<> const& arguments) const
	{
		...
		if (!op)
		{
			throw dis("dynamic parcel_d::release_unique_ passed non-existent member");
		}
		return op.operate(*const_cast<parcel_d*>(this), arguments);
	}
	...
	inline any_a < > release(shoal_a < > const & shared_shoal , any_a < > & unique_shoal ) const
	{ throw dis("dynamic parcel_d::release(shared_shoal, unique_shoal) not available"); }
