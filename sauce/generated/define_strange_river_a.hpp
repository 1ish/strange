...
protected:
	...
	struct ___river_a_handle___ : range_a< any_a<> >::template ___range_a_handle___<___TTT___, ___DHB___>
	{
		...
		virtual inline forward_extractor_data_a < any_a < > , std_istreambuf_iterator < char > > extract_end() const final;
...
private:
	...
	struct ___river_a_handle_final___ final : ___river_a_handle___<___TTT___>
	{
		...
		{
			return std_make_shared<___river_a_handle_final___>(___river_a_handle___<___TTT___>::___value___);
		}
	};
...
public:
	...
	template <typename ___TTT___>
	inline river_a& operator=(std_shared_ptr<___TTT___> const& handle) noexcept
	{
		assert(!handle || std::dynamic_pointer_cast<___river_a_handle_base___>(handle));
