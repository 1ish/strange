...
namespace strange {
    ...
    private:
      ...
      { return value_.reference__( ); }
    	...
    	friend inline bool check_(bidirectional_iterator_data_a<___1___> const& value) noexcept;
    
    public:
    	static inline char const* ___abstraction_name___() noexcept
    	{
    		return "bidirectional_iterator_data_a";
    	}
    	...
    	explicit inline bidirectional_iterator_data_a(bool reference) noexcept
    		: ___derived___{ reference }
    	{}
    
    	inline bidirectional_iterator_data_a(bidirectional_iterator_data_a const& other, bool reference) noexcept
    		: ___derived___(other, reference)
    	{}
    
    	inline bidirectional_iterator_data_a(bidirectional_iterator_data_a&& other, bool reference) noexcept
    		: ___derived___(std::move(other), reference)
    	{}
    
    #ifdef STRANGE_CHECK_STATIC_CASTS
    	template <typename ___TTT___>
    	explicit inline bidirectional_iterator_data_a(std::shared_ptr<___TTT___> const& handle, bool reference = false)
    		: ___derived___(handle, reference)
    	{
    		if (!std::dynamic_pointer_cast<___finale_handle_base___>(handle))
    		{
    			throw dis__("bidirectional_iterator_data_a constructor failed to cast from base to final");
    		}
    	}
    #else
    	template <typename ___TTT___>
    	explicit inline bidirectional_iterator_data_a(std::shared_ptr<___TTT___> const& handle, bool reference = false) noexcept
    		: ___derived___(handle, reference)
    	{
    	...
    	explicit inline bidirectional_iterator_data_a(___TTT___ value, bool reference = false) noexcept
    	...
    	inline bidirectional_iterator_data_a& operator=(std::shared_ptr<___TTT___> const& handle)
    	{
    		if (!std::dynamic_pointer_cast<___finale_handle_base___>(handle))
    		{
    			throw dis__("bidirectional_iterator_data_a assignment failed to cast from base to final");
    		}
    		handle_ = handle;
    		handle_->___weak___(handle_);
    		return *this;
    ...
    template <typename ___TTT___, typename ___1___>
    inline bool check_(bidirectional_iterator_data_a<___1___> const& value) noexcept
    {
