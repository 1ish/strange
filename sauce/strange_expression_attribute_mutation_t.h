...
public:
	...
	static inline expression_a<> create_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		{
			throw dis(token.report() + "strange::expression_attribute_mutation::create passed non-kind");
		}
		if (++it == terms.cend_())
		{
			throw dis(token.report() + "strange::expression_attribute_mutation::create passed too few terms");
		...
		return expression_substitute_t<over>::create(over{ expression_attribute_mutation_t<>(token, terms, cast<symbol_a<>>(name), cast<kind_a<>>(kind), cast<expression_a<>>(value)) },
...
protected:
	...
	inline expression_attribute_mutation_t(token_a<> const& token, flock_a<> const& terms, symbol_a<> const& name, kind_a<> const& result, expression_a<> const& expression)
