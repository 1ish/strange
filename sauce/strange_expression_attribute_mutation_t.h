...
public:
	...
	static inline expression_a<> val_(token_a<> const& token, flock_a<> const& terms)
	{
		if (terms.empty())
		{
			throw dis(token.report() + "strange::expression_attribute_mutation::val passed no terms");
		}
		any_a<> const term = terms.at_index(0);
		if (!check<expression_a<>>(term))
		{
			throw dis(token.report() + "strange::expression_attribute_mutation::val passed non-expression catch");
		}
		auto subterms = cast<expression_a<>>(term).terms_();
		if (subterms.size() != 3)
		{
			throw dis(token.report() + "strange::expression_attribute_mutation::val passed wrong number of subterms");
		}

		any_a<> const name = subterms.at_index(0);
		if (!check<symbol_a<>>(name))
		{
			throw dis(token.report() + "strange::expression_attribute_mutation::val passed non-symbol name");
		}

		any_a<> const cat = subterms.at_index(1);
		if (!check<cat_a<>>(cat))
		{
			throw dis(token.report() + "strange::expression_attribute_mutation::val passed non-cat");
		}
		...
		{
			throw dis(token.report() + "strange::expression_attribute_mutation::val passed wrong kind of thing");
		}
		return expression_substitute_t<over>::val(over{ expression_attribute_mutation_t<>(token, terms, cast<symbol_a<>>(name), cast<cat_a<>>(cat), cast<expression_a<>>(value)) },
			attribute_mutation_t<>::val_(cast<symbol_a<>>(name), thing));
	}
