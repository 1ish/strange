...
public:
	...
	inline any_a<> share__(range_a<> const& range) const
	{
		...
		{
			throw dis("strange::expression_creature::share passed non-shoal");
		}
		reflection<expression_c<>>::share(cast<shoal_a<>>(thing, true));
		return thing;
	}
	...
	inline shoal_a<> share_(shoal_a<>& shoal) const
	{
		...
		if (op)
		{
			...
			{
				throw dis("strange::expression_creature::share returned non-shoal");
			}
