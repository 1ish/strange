...
public:
	...
	static inline expression_a<> create_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		{
			throw dis(__FILE__, __LINE__, token.report() + "strange::expression_cast::create not passed any terms");
		}
		any_a<> scope = *it;
		if (!check<symbol_a<>>(scope))
		{
			throw dis(__FILE__, __LINE__, token.report() + "strange::expression_cast::create passed non-symbol scope");
		}
		if (++it == terms.extract_end_())
		{
			throw dis(__FILE__, __LINE__, token.report() + "strange::expression_cast::create passed too few terms");
		}
		any_a<> thing = *it;
		if (!check<expression_a<>>(thing))
		{
			throw dis(__FILE__, __LINE__, token.report() + "strange::expression_cast::create passed non-expression thing term");
		}
		if (++it == terms.extract_end_())
		{
		...
		{
			throw dis(__FILE__, __LINE__, token.report() + "strange::expression_cast::create passed non-expression abstraction term");
		}
	...
	inline any_a<> operate(any_a<>& thing, list_a<> const& list) const
	{
		...
		{
			throw dis(__FILE__, __LINE__, expression_t<___ego___>::_token.report() + "strange::expression_cast::operate check failed");
		}
#endif
		return evaluated;
	}
	...
	inline void generate_cpp(int64_t version, int64_t indent, river_a<>& river, bool declare, bool define, bool type = false) const
	{
		...
		if (type)
		{
			throw dis(__FILE__, __LINE__, expression_t<___ego___>::_token.report() + "strange::expression_cast::generate_cpp called for wrong type of expression");
		}
