...
public:
	...
	static inline expression_a<> create_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		return expression_substitute_t<over>::create(over{ expression_extraction_t<>(token, terms, cast<symbol_a<>>(scope), names, kinds, expressions, defaults, cast<expression_a<>>(expression)) },
			extraction_t<>::create_(token, cast<symbol_a<>>(scope), names, kinds, defaults, cast<expression_a<>>(expression)));
	}
	...
	inline void generate_cpp(int64_t version, int64_t indent, river_a<>& river, bool declare, bool define, bool type = false) const
	{
		...
		if (type)
		{
			throw dis(_token.report() + "strange::expression_extraction::generate_cpp called for wrong type of expression");
		}
		river.write_string(" [](");
		auto nit = _names.extract_vector().cbegin();
		auto kit = _kinds.extract_vector().cbegin();
