...
public: ___STRANGE_EXPRESSION___
	...
	{
		return expression_a<>{ expression_flock_t<>{ token, terms } };
	}
	...
	inline void generate(int64_t indent, river_a<>& river) const
	{
		...
		for (auto const& term : _terms)
		{
			...
			{
				throw dis(_token.report() + "strange::expression_flock::generate passed non-expression term");
			}
	...
	{
		river.write_string(" flock_t<>::var(");
		bool first = true;
		for (auto const& term : _terms)
		{
			if (first)
			...
			if (!check<expression_a<>>(term))
			{
				throw dis(_token.report() + "strange::expression_flock::generate_cpp passed non-expression term");
			}
