...
public:
	...
	static inline expression_a<> create_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		for (auto const& term : terms)
		{
			...
			{
				throw dis(token.report() + "strange::expression_function::create passed non-expression catch");
			}
			...
			if (!check<cat_a<>>(cat))
			{
				throw dis(token.report() + "strange::expression_function::create passed non-cat");
			}
		...
		return expression_substitute_t<over>::create(over{ expression_function_t<>(token, terms, names, params, values, defaults, cast<symbol_a<>>(name), cast<cat_a<>>(cat), cast<expression_a<>>(value)) },
			function_t<>::create_(token, names, params, defaults, cast<symbol_a<>>(name), cast<cat_a<>>(cat), cast<expression_a<>>(value)));
	}
