...
public:
	...
	static inline expression_a<> val_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		if (it == terms.cend_())
		{
			throw dis(token.report() + "strange::expression_intimate_attribute::val passed empty range");
		}
		auto thing = *it;
		if (!check<expression_a<>>(thing))
		{
			throw dis(token.report() + "strange::expression_intimate_attribute::val passed non-expression thing term");
		}
		if (++it == terms.cend_())
		{
			throw dis(token.report() + "strange::expression_intimate_attribute::val passed short range");
		}
		auto member = *it;
		if (!check<symbol_a<>>(member))
		{
			throw dis(token.report() + "strange::expression_intimate_attribute::val passed non-symbol member term");
		}
		if (++it == terms.cend_())
		{
			return expression_substitute_t<over>::val(over{ expression_intimate_attribute_t<>(token, terms, cast<expression_a<>>(thing), cast<symbol_a<>>(member)) });
		}
		auto value = *it;
		if (!check<expression_a<>>(value))
		{
			throw dis(token.report() + "strange::expression_intimate_attribute::val passed non-expression value term");
		}
		return expression_substitute_t<over>::val(over{ expression_intimate_attribute_t<>(token, terms, cast<expression_a<>>(thing), cast<symbol_a<>>(member), cast<expression_a<>>(value)) });
	}
	...
	inline any_a<> operate(any_a<>& thing, range_a<> const& range) const
	{
		...
		if (_assign)
		{
			return any_c<>::intimate(thing_term, _member, flock_t<>::val_(_value.operate(thing, range)));
		}
		return any_c<>::intimate(thing_term, _member, flock_t<>::val_());
	}
...
protected:
	...
	static inline std::pair<bool, bool> is_pure_literal(token_a<> const& token, flock_a<> const& terms, expression_a<> const& expression_thing, symbol_a<> const& member, expression_a<> const& expression_value)
	{
		...
		try
		{
			...
			{
				throw dis(token.report() + "strange::expression_intimate_attribute::val passed non-existent member");
			}
			auto any_operation = thing.operations_().at_(member);
			if (!check<operation_a<>>(any_operation))
			{
				throw dis(token.report() + "strange::expression_intimate_attribute::val passed non-operation member");
			}
		...
		{
			throw dis("strange::expression_intimate_attribute::val pure literal evaluation error:") + token.report_() + misunderstanding;
		}
