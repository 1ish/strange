...
public:
	...
	static inline expression_a<> val_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		{
			throw dis(token.report() + "strange::expression_intimate_member_range::val passed non-expression member term");
		}
		if (++it == terms.cend_())
		{
			throw dis(token.report() + "strange::expression_intimate_member_range::val passed short range");
		...
		return expression_substitute_t<over>::val(over{ expression_intimate_member_range_t<>(token, terms, cast<expression_a<>>(thing), cast<expression_a<>>(member), cast<expression_a<>>(range)) });
	}
	...
	inline any_a<> operate(any_a<>& thing, range_a<> const& range) const
	{
		...
		return any_c<>::intimate(thing_term, member_term, cast<range_a<>>(range_term));
	}
