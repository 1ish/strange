...
protected:
	...
	static inline std::pair<bool, bool> is_pure_literal(token_a<> const& token, expression_a<> const& thing_expression, symbol_a<> const& member)
	{
		...
		try
		{
			...
			{
				throw dis(token.report() + "strange::expression_intimate_member::val passed non-existent member");
			}
			auto any_operation = thing.operations_().at_(member);
			if (!check<operation_a<>>(any_operation))
			{
				throw dis(token.report() + "strange::expression_intimate_member::val passed non-operation member");
			}
		...
		{
			throw dis("strange::expression_intimate_member::val pure literal evaluation error:") + token.report_() + misunderstanding;
		}
