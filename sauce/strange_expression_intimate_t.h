...
class expression_intimate_t : public expression_t<_ABSTRACTION_>
{
public:
	// override
	...
	{
		return expression_substitute_t<over>::val(over{ expression_intimate_t<>(token, terms) });
	}
	...
	{
		return any_c<>::intimate__(range_operator_t<>::val_(_terms, thing, range));
	}
...
protected:
	...
	static inline std::pair<bool, bool> is_pure_literal(token_a<> const& token, flock_a<> const& terms)
	{
		...
		if (it == terms.cend_())
		{
			throw dis(token.report() + "strange::expression_intimate::val passed no terms");
		}
		auto any_thing = *it;
		if (!check<expression_a<>>(any_thing))
		{
			throw dis(token.report() + "strange::expression_intimate::val passed non-expression thing");
		}
		...
		if (++it == terms.cend_())
		{
			throw dis(token.report() + "strange::expression_intimate::val passed too few terms");
		}
		auto any_member = *it;
		if (!check<expression_a<>>(any_member))
		{
			throw dis(token.report() + "strange::expression_intimate::val passed non-expression member");
		}
		...
		while (++it != terms.cend_())
		{
			...
			if (!check<expression_a<>>(term))
			{
				throw dis(token.report() + "strange::expression_intimate::val passed non-expression argument");
			}
...
bool const expression_intimate_t<_ABSTRACTION_>::___share___ = []()
{
	auto shoal = shoal_a<>(shared(), true);
	expression_intimate_t<_ABSTRACTION_>::share(shoal);
	return shoal;
}();
