...
protected:
	...
	static inline std::pair<bool, bool> is_pure_literal(token_a<> const& token, expression_a<> const& thing_expression, symbol_a<> const& member, expression_a<> const& range_expression)
	{
		...
		try
		{
			...
			{
				throw dis(token.report() + "strange::expression_invoke_member_range::create passed non-range");
			}
			auto range = cast<range_a<>>(any_range);
			if (!thing.operations_().has_(member))
			{
				throw dis(token.report() + "strange::expression_invoke_member_range::create passed non-existent member");
			}
			auto any_operation = thing.operations_().at_(member);
			if (!check<operation_a<>>(any_operation))
			{
				throw dis(token.report() + "strange::expression_invoke_member_range::create passed non-operation member");
			}
		...
		{
			throw dis("strange::expression_invoke_member_range::create pure literal evaluation error:") + token.report_() + misunderstanding;
		}
