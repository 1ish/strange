...
public: ___STRANGE_EXPRESSION___
	...
	static inline expression_a<> val_(token_a<> const& token, range_a<> const& terms)
	{
		...
		return expression_override_t<expression_invoke_range_t<>>::val(expression_invoke_range_t<>(token, cast<expression_a<>>(thing), cast<expression_a<>>(range)));
	}
...
protected:
	...
	static inline std::pair<bool, bool> is_pure_literal(token_a<> const& token, expression_a<> const& thing_expression, expression_a<> const& range_expression)
	{
		...
		{
			throw dis("strange::expression_invoke_range::val passed non-range");
		}
		auto range = cast<range_a<>>(any_range);
		auto it = range.cbegin_();
		if (it == range.cend_())
		{
			throw dis("strange::expression_invoke_range::val passed empty range");
		}
		auto member = *it;
		if (!thing.operations_().has_(member))
		{
			throw dis("strange::expression_invoke_range::val passed non-existent member");
		}
		auto any_operation = thing.operations_().at_(member);
		if (!check<operation_a<>>(any_operation))
		{
			throw dis("strange::expression_invoke_range::val passed non-operation member");
		}
