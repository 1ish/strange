...
public:
	...
	static inline expression_a<> val_(token_a<> const& token, range_a<> const& terms)
	{
		...
		return expression_substitute_t<over>::val(over{ expression_invoke_range_t<>(token, cast<expression_a<>>(thing), cast<expression_a<>>(range)) });
	}
...
protected:
	...
	inline expression_invoke_range_t(token_a<> const& token, expression_a<> const& thing, expression_a<> const& range)
