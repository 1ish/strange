...
public:
	...
	static inline any_a<> validate__(list_a<> const& list)
	{
		...
		if (it == list.end_())
		{
			throw dis(__FILE__, __LINE__, "strange::expression_literal::validate passed empty list");
		}
		return validate_(*it);
	}
	...
	{
		return expression_t<___ego___>::template recreate_expression<expression_literal_t<___ego___>>(expression_t<___ego___>::me_());
	}
	...
	inline void generate(int64_t version, int64_t indent, river_a<>& river) const
	{
		...
		{
			river.write_string(" $$strange::number_int_8::create[" + std_to_string(fast<number_data_a<int8_t>>(_thing).extract_primitive()) + "] ");
		}
		else if	(_thing.type_() == number_uint_8_t<>::type_())
		{
			river.write_string(" $$strange::number_uint_8::create[" + std_to_string(fast<number_data_a<uint8_t>>(_thing).extract_primitive()) + "] ");
		}
		else if (_thing.type_() == number_int_16_t<>::type_())
		{
			river.write_string(" $$strange::number_int_16::create[" + std_to_string(fast<number_data_a<int16_t>>(_thing).extract_primitive()) + "] ");
		}
		else if (_thing.type_() == number_uint_16_t<>::type_())
		{
			river.write_string(" $$strange::number_uint_16::create[" + std_to_string(fast<number_data_a<uint16_t>>(_thing).extract_primitive()) + "] ");
		}
		else if (_thing.type_() == number_int_32_t<>::type_())
		{
			river.write_string(" $$strange::number_int_32::create[" + std_to_string(fast<number_data_a<int32_t>>(_thing).extract_primitive()) + "] ");
		}
		else if (_thing.type_() == number_uint_32_t<>::type_())
		{
			river.write_string(" $$strange::number_uint_32::create[" + std_to_string(fast<number_data_a<uint32_t>>(_thing).extract_primitive()) + "] ");
		}
		else if (_thing.type_() == number_int_64_t<>::type_())
		{
			river.write_string(" " + std_to_string(fast<number_data_a<int64_t>>(_thing).extract_primitive()) + " ");
		}
		else if (_thing.type_() == number_uint_64_t<>::type_())
		{
			river.write_string(" $$strange::number_uint_64::create[" + std_to_string(fast<number_data_a<uint64_t>>(_thing).extract_primitive()) + "] ");
		}
		else if (_thing.type_() == number_float_32_t<>::type_())
		{
			river.write_string(" $$strange::number_float_32::create[" + std_to_string(fast<number_data_a<float>>(_thing).extract_primitive()) + "] ");
		}
		else if (_thing.type_() == number_float_64_t<>::type_())
		{
			river.write_string(" " + std_to_string(fast<number_data_a<double>>(_thing).extract_primitive()) + " ");
		}
		else
		{
	...
	inline void generate_cpp(int64_t version, int64_t indent, river_a<>& river, bool declare, bool define, bool type = false) const
	{
		...
		else if (_thing.type_() == number_int_64_t<>::type_())
		{
			...
			{
				river.write_string(" strange::number_int_64::create(" + std_to_string(fast<number_data_a<int64_t>>(_thing).extract_primitive()) + ") ");
			}
		}
		else if (_thing.type_() == number_uint_64_t<>::type_())
		{
		...
		else if (_thing.type_() == number_float_64_t<>::type_())
		{
			...
			{
				river.write_string(" strange::number_float_64::create(" + std_to_string(fast<number_data_a<double>>(_thing).extract_primitive()) + ") ");
			}
		}
		else
		{
