...
public:
	...
	{
		return expression_t<___ego___>::template create_expression<expression_local_insert_t<___ego___>>(range);
	}
	...
	static inline expression_a<> create_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		if (++it == terms.extract_end_())
		{
			return expression_a<>::create<expression_local_insert_t<>>(token, terms, fast<symbol_a<>>(key), kind, expression_t<>::create(token));
		}
		any_a<> expression = *it;
	...
	{
		shoal.update_string("strange::expression_local_insert::create", native_function_create(&expression_local_insert_t<>::create__));
	}
	...
	{
		return expression_t<___ego___>::template recreate_expression<expression_local_insert_t<___ego___>>(expression_t<___ego___>::me_());
	}
