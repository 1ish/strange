...
public:
	...
	static inline expression_a<> create_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		while (!at_end)
		{
			...
			{
				throw dis(__FILE__, __LINE__, "strange::expression_mutation::create parameter default evaluation error:") + token.report_() + misunderstanding;
			}
		}
		return expression_substitute_t<expression_mutation_t<>>::create(
			mutation_t<>::create_(token, fast<symbol_a<>>(scope), names, kinds, defaults, fast<expression_a<>>(expression)),
			expression_mutation_t<>(token, terms, fast<symbol_a<>>(scope), names, kinds, expressions, defaults, fast<expression_a<>>(expression)));
	}
	...
	{
		shoal.update(sym("strange::expression_mutation::create"), native_function_create(&expression_mutation_t<>::create__));
	}

	// expression
	inline expression_a<> recreate_() const
	{
		return expression_t<___ego___>::template recreate_expression<expression_mutation_t<___ego___>>(expression_t<___ego___>::me_());
	}
