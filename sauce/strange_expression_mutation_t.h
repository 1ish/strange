...
public:
	...
	static inline expression_a<> create_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		for (auto const& term : terms)
		{
			...
			{
				throw dis(token.report() + "strange::expression_mutation::create passed non-expression catch");
			}
			auto subterms = cast<expression_a<>>(term).terms_();
			if (subterms.size() != 3)
			{
				throw dis(token.report() + "strange::expression_mutation::create passed wrong number of subterms");
			}
			...
			if (!check<cat_a<>>(cat))
			{
				throw dis(token.report() + "strange::expression_mutation::create passed non-cat");
			}
		...
		return expression_substitute_t<over>::create(over{ expression_mutation_t<>(token, terms, names, params, values, defaults, cast<symbol_a<>>(name), cast<cat_a<>>(cat), cast<expression_a<>>(value)) },
			mutation_t<>::create_(token, names, params, defaults, cast<symbol_a<>>(name), cast<cat_a<>>(cat), cast<expression_a<>>(value)));
	}
