...
class expression_mutation_t : public expression_t<_ABSTRACTION_>
{
public:
	// override
	...
	inline any_a<> operate_(any_a<>& thing, range_a<> const& range) const
	{
		...
		for (auto const& def : _defaults.extract())
		{
			...
			{
				throw dis(_token.report() + "strange::expression_mutation::operate cat does not include argument");
			}
...
bool const expression_mutation_t<_ABSTRACTION_>::___share___ = []()
{
	auto shoal = shoal_a<>(shared(), true);
	expression_mutation_t<_ABSTRACTION_>::share(shoal);
	return shoal;
}();
