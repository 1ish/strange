...
public:
	...
	static inline expression_a<> val_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		if (it == terms.cend_())
		{
			throw dis(token.report() + "strange::expression_nand::val not passed any terms");
		}
		any_a<> left = *it;
		if (!check<expression_a<>>(left))
		{
			throw dis(token.report() + "strange::expression_nand::val passed non-expression left-hand term");
		}
		if (++it == terms.cend_())
		{
			throw dis(token.report() + "strange::expression_nand::val not passed sufficient terms");
		}
		any_a<> right = *it;
		if (!check<expression_a<>>(right))
		{
			throw dis(token.report() + "strange::expression_nand::val passed non-expression right-hand term");
		}
