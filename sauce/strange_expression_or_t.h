...
public:
	// override
	using over = expression_o<expression_or_t<>>;
	...
	static inline expression_a<> val_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		if (it == terms.cend_())
		{
			throw dis(token.report() + "strange::expression_or::val not passed any terms");
		}
		any_a<> left = *it;
		if (!check<expression_a<>>(left))
		{
			throw dis(token.report() + "strange::expression_or::val passed non-expression left-hand term");
		}
		if (++it == terms.cend_())
		{
			throw dis(token.report() + "strange::expression_or::val not passed sufficient terms");
		}
		any_a<> right = *it;
		if (!check<expression_a<>>(right))
		{
			throw dis(token.report() + "strange::expression_or::val passed non-expression right-hand term");
		}
		return expression_a<>{ over{ expression_or_t<>( token, terms, cast<expression_a<>>(left), cast<expression_a<>>(right)) } };
	}

	// reflection
	static inline symbol_a<> type_()
	{
		return reflection<expression_or_t<>>::type();
	}

	static inline void share(shoal_a<>& shoal)
	{
		reflection<expression_or_t<>>::share(shoal);
	}
