...
public:
	...
	static inline expression_a<> create_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		return expression_a<>{ over{ expression_pipe_t<>( token, terms, cast<expression_a<>>(left), cast<expression_a<>>(right)) } };
	}

	// reflection
	static inline symbol_a<> type_()
	{
		return reflection<expression_pipe_t<>>::type();
	}

	static inline void share(shoal_a<>& shoal)
	{
		reflection<expression_pipe_t<>>::share(shoal);
	}
	...
	inline void generate_cpp(int64_t version, int64_t indent, river_a<>& river, bool type = false) const
	{
		if (type)
		{
			throw dis(_token.report() + "strange::expression_pipe::generate_cpp called for wrong type of expression");
		}
		river.write_string(" (");
