...
public:
	...
	static inline expression_a<> create_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		{
			throw dis(__FILE__, __LINE__, token.report() + "strange::expression_return::create not passed any terms");
		}
		any_a<> scope = *it;
		if (!check<symbol_a<>>(scope))
		{
			throw dis(__FILE__, __LINE__, token.report() + "strange::expression_return::create passed non-symbol scope");
		}
		if (++it == terms.extract_end_())
		{
		...
		{
			throw dis(__FILE__, __LINE__, "strange::expression_return::create passed non-expression result");
		}
	...
	inline void generate_cpp(int64_t version, int64_t indent, river_a<>& river, bool declare, bool define, bool type = false) const
	{
		...
		if (type)
		{
			throw dis(__FILE__, __LINE__, expression_t<___ego___>::_token.report() + "strange::expression_return::generate_cpp called for wrong type of expression");
		}
		river.write_string(" return(");
		_result.generate_cpp(version, indent, river, declare, define);
		river.write_string("); ");
