...
public:
	...
	static inline expression_a<> val_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		{
			throw dis(token.report() + "strange::expression_shared_insert::val passed non-symbol key");
		}
		if (++it == terms.cend_())
		{
...
bool const expression_shared_insert_t<_ABSTRACTION_>::___share___ = []()
{
	auto shoal = shoal_a<>(shared(), true);
	expression_shared_insert_t<_ABSTRACTION_>::share(shoal);
	return shoal;
}();
