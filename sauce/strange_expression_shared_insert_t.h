...
public: ___STRANGE_EXPRESSION___
	...
	static inline expression_a<> val_(range_a<> const& terms)
	{
		...
		return expression_a<>{ expression_shared_insert_t<>{ cast<cat_a<>>(cat), key, cast<expression_a<>>(val) } };
	}
...
protected:
	...
	inline expression_shared_insert_t(cat_a<> const& cat, any_a<> const& key, expression_a<> const& val)
		: expression_t{}
		, _cat{ cat }
		, _key{ key }
		, _val{ val }
