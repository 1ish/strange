...
public:
	...
	static inline expression_a<> val_(token_a<> const& token, range_a<> const& terms)
	{
		...
		return expression_a<>{ over{ expression_shared_insert_t<>{ token, cast<cat_a<>>(cat), key, cast<expression_a<>>(val) } } };
	}
...
protected:
	...
	inline expression_shared_insert_t(token_a<> const& token, cat_a<> const& cat, any_a<> const& key, expression_a<> const& val)
		: expression_t{ token }
		, _cat{ cat }
		, _key{ key }
		, _val{ val }
