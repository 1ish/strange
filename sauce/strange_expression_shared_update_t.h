...
public:
	...
	static inline expression_a<> val_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		return expression_a<>{ over{ expression_shared_update_t<>{ token, terms, cast<cat_a<>>(cat), key, cast<expression_a<>>(val) } } };
	}
...
protected:
	...
	inline expression_shared_update_t(token_a<> const& token, flock_a<> const& terms, cat_a<> const& cat, any_a<> const& key, expression_a<> const& val)
		: expression_t{ token }
		, _terms{ terms }
		, _cat{ cat }
		, _key{ key }
