...
public: ___STRANGE_EXPRESSION___
	...
	static inline expression_a<> val_(range_a<> const& terms)
	{
		...
		if (it == terms.cend_())
		{
			throw dis("strange::expression_shared_update::val not passed any terms");
		}
		any_a<> cat = *it;
		if (!check<cat_a<>>(cat))
		{
			throw dis("strange::expression_shared_update::val passed non-cat");
		}
		if (++it == terms.cend_())
		{
			throw dis("strange::expression_shared_update::val not passed sufficient terms");
		}
		any_a<> key = *it;
		if (++it == terms.cend_())
		{
		...
		{
			throw dis("strange::expression_shared_update::val passed non-expression");
		}
		return expression_a<>{ expression_shared_update_t<>{ cast<cat_a<>>(cat), key, cast<expression_a<>>(val) } };
	}
	...
	inline any_a<> operate_(any_a<>& thing, range_a<> const& range) const
	{
		...
		{
			throw dis("strange::expression_shared_update::operate passed non-unordered-shoal local");
		}
#endif
		...
		if (lit == local.end())
		{
			throw dis("strange::expression_shared_update::operate $ not found");
		}
#ifdef STRANGE_CHECK_STATIC_CASTS
		if (!check<unordered_shoal_a<>>(lit->second))
		{
			throw dis("strange::expression_shared_update::operate passed non-unordered-shoal shared");
		}
#endif
		...
		if (sit == shared.end())
		{
			throw dis("strange::expression_shared_update::operate key not found");
		}
		auto val = _val.operate_(thing, range);
		if (!val.cats_().has_(_cat))
		{
			throw dis("strange::expression_shared_update::operate cat does not include value");
		}
	...
	inline void generate_cpp(int64_t indent, river_a<>& river) const
	{
		if (!check<symbol_a<>>(_key))
		{
			throw dis("strange::expression_shared_update::generate_cpp called with non-symbol key");
		}
		river.write_(lake_from_string(" " + cast<symbol_a<>>(_key).to_string() + " ="));
...
protected:
	...
	inline expression_shared_update_t(cat_a<> const& cat, any_a<> const& key, expression_a<> const& val)
		: expression_t{}
		, _cat{ cat }
		, _key{ key }
		, _val{ val }
