...
public:
	...
	static inline expression_a<> val_(token_a<> const& token, flock_a<> const& terms)
	{
		...
		return expression_a<>{ over{ expression_shared_update_t<>{ token, terms, key, cast<cat_a<>>(cat), cast<expression_a<>>(val) } } };
	}
...
protected:
	...
	inline expression_shared_update_t(token_a<> const& token, flock_a<> const& terms, any_a<> const& key, cat_a<> const& cat, expression_a<> const& val)
		: expression_t{ token }
		, _terms{ terms }
		, _key{ key }
		, _cat{ cat }
