...
public:
	...
	{
		shoal.update(sym("strange::expression_shoal::create"), native_function_create(&expression_shoal_t<>::create__));
	}
	...
	{
		return expression_t<___ego___>::template recreate_expression<expression_shoal_t<___ego___>>(expression_t<___ego___>::me_());
	}
	...
	inline void generate(int64_t version, int64_t indent, river_a<>& river) const
	{
		...
		for (auto const& term : _terms.extract_vector())
		{
			...
			{
				throw dis(__FILE__, __LINE__, expression_t<___ego___>::_token.report() + "strange::expression_shoal::generate with non-expression term");
			}
	...
	inline void generate_cpp(int64_t version, int64_t indent, river_a<>& river, bool declare, bool define, bool type = false) const
	{
		if (declare || define)
		{
			for (auto const& term : _terms.extract_vector())
			{
				if (!check<expression_a<>>(term))
				{
					throw dis(__FILE__, __LINE__, expression_t<___ego___>::_token.report() + "strange::expression_shoal::generate_cpp with non-expression term");
				}
		...
		if (type)
		{
			throw dis(__FILE__, __LINE__, expression_t<___ego___>::_token.report() + "strange::expression_shoal::generate_cpp called for wrong type of expression");
		}
