...
public:
	...
	{
		return reflection<expression_xor_t<>>::type();
	}

	static inline void share(shoal_a<>& shoal)
	{
		reflection<expression_xor_t<>>::share(shoal);
	}
	...
	inline void generate_cpp(int64_t version, int64_t indent, river_a<>& river, bool type = false) const
	{
		if (type)
		{
			throw dis(_token.report() + "strange::expression_xor::generate_cpp called for wrong type of expression");
		}
		river.write_string(" (!");
