...
namespace strange {
    ...
    class forward_const_iterator_a : public ___root___
    {
    public:
    	...
    	inline forward_const_iterator_a& operator+=(___root___ const& other)
    	{
    		assert(handle_);
    ...
    private:
    	...
    	template <typename ___TTT___>
    	friend inline bool check(forward_const_iterator_a<> const& value) noexcept;
    ...
    public:
    	...
    	inline forward_const_iterator_a(forward_const_iterator_a& other, reference_tag) noexcept
    	...
    	{
    		return forward_const_iterator_a(other, reference_tag{});
    	}
    
    	inline forward_const_iterator_a(forward_const_iterator_a& other, duplicate_tag) noexcept
    	...
    	{
    		return forward_const_iterator_a(other, duplicate_tag{});
    	}
    		...
    		: ___root___{ handle }
    	{
    		...
    		{
    			throw dis("forward_const_iterator_a constructor failed to cast from base to derived");
    		}
    	}
    #else
    	template <typename ___TTT___>
    ...
    #ifdef STRANGE_CHECK_STATIC_CASTS
    	template <typename ___TTT___>
    	explicit inline forward_const_iterator_a(std::shared_ptr<___TTT___> const& handle, bool reference)
    ...
    #else
    	template <typename ___TTT___>
    	explicit inline forward_const_iterator_a(std::shared_ptr<___TTT___> const& handle, bool reference) noexcept
    		: ___root___(handle, reference)
    	{
    		assert(!handle || std::dynamic_pointer_cast<___derived_handle_base___>(handle));
    	}
    ...
    #ifdef STRANGE_CHECK_STATIC_CASTS
    	template <typename ___TTT___>
    	explicit inline forward_const_iterator_a(std::shared_ptr<___TTT___>& handle, reference_tag)
    ...
    #else
    	template <typename ___TTT___>
    	explicit inline forward_const_iterator_a(std::shared_ptr<___TTT___>& handle, reference_tag) noexcept
    		: ___root___(handle, reference_tag{})
    	{
    		assert(!handle || std::dynamic_pointer_cast<___derived_handle_base___>(handle));
    	}
    	...
    	inline forward_const_iterator_a& operator=(std::shared_ptr<___TTT___> const& handle)
    	{
    		if (handle && !std::dynamic_pointer_cast<___derived_handle_base___>(handle))
    		{
    			throw dis("forward_const_iterator_a assignment failed to cast from base to derived");
    		}
    		handle_ = handle;
    		return *this;
    	}
    ...
    template <typename ___TTT___>
    inline bool check(forward_const_iterator_a<> const& value) noexcept
    {
    	return ___TTT___::___check___(value.handle_);
    }
    
    template <typename ___1___>
    bool const forward_const_iterator_a<___1___>::___share___ = []()
    {
    	auto& shoal = shared();
