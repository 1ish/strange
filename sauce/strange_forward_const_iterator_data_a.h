...
namespace strange {
    ...
    class forward_const_iterator_data_a final : public ___derived___
    {
    public:
    	...
    	inline forward_const_iterator_data_a& operator+=(___root___ const& other)
    	{
    		assert(handle_);
    ...
    private:
      ...
      { return value_.mutate_it( ); }
    	...
    	friend inline bool check(forward_const_iterator_data_a<___1___> const& value) noexcept;
    ...
    public:
    	...
    	inline forward_const_iterator_data_a(forward_const_iterator_data_a const& other, bool reference) noexcept
    		: ___derived___(other, reference)
    	{}
    
    	inline forward_const_iterator_data_a(forward_const_iterator_data_a& other, reference_tag) noexcept
    	...
    	{
    		return forward_const_iterator_data_a(other, reference_tag{});
    	}
    
    	inline forward_const_iterator_data_a(forward_const_iterator_data_a& other, duplicate_tag) noexcept
    	...
    	{
    		return forward_const_iterator_data_a(other, duplicate_tag{});
    	}
    		...
    		: ___derived___{ handle }
    	{
    		...
    		{
    			throw dis("forward_const_iterator_data_a constructor failed to cast from base to final");
    		}
    	}
    #else
    	template <typename ___TTT___>
    ...
    #ifdef STRANGE_CHECK_STATIC_CASTS
    	template <typename ___TTT___>
    	explicit inline forward_const_iterator_data_a(std::shared_ptr<___TTT___> const& handle, bool reference)
    ...
    #else
    	template <typename ___TTT___>
    	explicit inline forward_const_iterator_data_a(std::shared_ptr<___TTT___> const& handle, bool reference) noexcept
    		: ___derived___(handle, reference)
    	{
    		assert(!handle || std::dynamic_pointer_cast<___finale_handle_base___>(handle));
    	}
    ...
    #ifdef STRANGE_CHECK_STATIC_CASTS
    	template <typename ___TTT___>
    	explicit inline forward_const_iterator_data_a(std::shared_ptr<___TTT___>& handle, reference_tag)
    ...
    #else
    	template <typename ___TTT___>
    	explicit inline forward_const_iterator_data_a(std::shared_ptr<___TTT___>& handle, reference_tag) noexcept
    		: ___derived___(handle, reference_tag{})
    	{
    		assert(!handle || std::dynamic_pointer_cast<___finale_handle_base___>(handle));
    	}
    ...
    template <typename ___TTT___, typename ___1___>
    inline bool check(forward_const_iterator_data_a<___1___> const& value) noexcept
    {
    	return ___TTT___::___check___(value.handle_);
    }
    
    template <typename ___1___>
    bool const forward_const_iterator_data_a<___1___>::___share___ = []()
    {
    	auto& shoal = shared();
