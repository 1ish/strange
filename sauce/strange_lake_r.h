...
class reflection<lake_a<PRIMITIVE>>
{
public:
	static inline cat_a<> cat()
	{
		static cat_a<> CAT = cat_t<>::val(1, "strange::lake_" + number_u<PRIMITIVE>::category());
		return CAT;
	}
...
class reflection<lake_t<PRIMITIVE, CONCURRENT>>
{
public:
	static inline symbol_a<> type()
	{
		static symbol_a<> TYPE = sym("strange::lake_" + number_u<PRIMITIVE>::category() +
	...
	static inline void share(shoal_a<>& shoal)
	{
		shoal.update_string(type().to_string() + "::val", native_function_t<>::val(&lake_t<PRIMITIVE, CONCURRENT>::val__));
	}
};
