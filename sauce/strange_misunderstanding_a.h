...
namespace strange {
    ...
    #ifdef STRANGE_CHECK_STATIC_CASTS
    	template <typename ___TTT___>
    	explicit inline misunderstanding_a(std::shared_ptr<___TTT___>& handle, reference_tag)
    ...
    #else
    	template <typename ___TTT___>
    	explicit inline misunderstanding_a(std::shared_ptr<___TTT___>& handle, reference_tag) noexcept
    		: ___root___(handle, reference_tag{})
    	{
    		assert(!handle || std::dynamic_pointer_cast<___derived_handle_base___>(handle));
    	}
