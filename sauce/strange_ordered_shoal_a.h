...
namespace strange {
    ...
    public:
     ...
     inline std :: map < any_a < > , any_a < >> const & extract ( ) const
     { assert(handle_); return read().extract( ); }
     inline void mutate ( std :: map < any_a < > , any_a < >> const & data )
     { assert(handle_); write().mutate(data ); }
     inline std :: map < any_a < > , any_a < >> & reference ( )
     { assert(handle_); return write().reference( ); }
    
    private:
    	struct ___finale_handle_base___ : ___ddderived_handle_base___
    	{
      virtual inline std :: map < any_a < > , any_a < >> const & extract ( ) const = 0;
      virtual inline void mutate ( std :: map < any_a < > , any_a < >> const & data ) = 0;
      virtual inline std :: map < any_a < > , any_a < >> & reference ( ) = 0;
    	};
      ...
      virtual inline std :: map < any_a < > , any_a < >> const & extract ( ) const
      { return value_.extract( ); }
      virtual inline void mutate ( std :: map < any_a < > , any_a < >> const & data )
      { value_.mutate(data ); }
      virtual inline std :: map < any_a < > , any_a < >> & reference ( )
      { return value_.reference( ); }
    	...
    	template <typename ___TTT___>
    	friend inline bool check_(ordered_shoal_a<> const& value) noexcept;
    
    public:
    	static inline char const* ___abstraction_name___() noexcept
    	{
    		return "ordered_shoal_a";
    	}
    	...
    	explicit inline ordered_shoal_a(bool reference) noexcept
    	...
    	template <typename ___TTT___>
    	explicit inline ordered_shoal_a(std::shared_ptr<___TTT___> const& handle, bool reference = false)
    ...
    #else
    	template <typename ___TTT___>
    	explicit inline ordered_shoal_a(std::shared_ptr<___TTT___> const& handle, bool reference = false) noexcept
    		: ___derived___(handle, reference)
    	{
    	...
    	explicit inline ordered_shoal_a(___TTT___ value, bool reference = false) noexcept
    ...
    template <typename ___TTT___>
    inline bool check_(ordered_shoal_a<> const& value) noexcept
    {
