...
public: ___STRANGE_COLLECTION___
	...
	static inline symbol_a<> type_()
	{
		return reflection<ordered_shoal_t<CONCURRENT>>::type();
	}

	static inline void share(shoal_a<>& shoal)
	{
		reflection<ordered_shoal_t<CONCURRENT>>::share(shoal);
	}
	...
	inline any_a<> visit__(range_a<> const& range) const
	{
		...
		{
			throw dis("strange::ordered_shoal::visit passed non-flock");
		}
