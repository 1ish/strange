...
public: ___STRANGE_COLLECTION___
	...
	{
		return cast_<squad_a<>>(val_() += range);
	}
	...
	{
		return cast_<squad_a<>>(ref_() += range, true);
	}
	...
	inline bool operator==(any_a<> const& thing) const
	{
		if (!check_<squad_a<>>(thing))
		{
			return false;
		}
		return _deque == cast_<squad_a<>>(thing).extract();
	}
	...
	{
		if (check_<squad_a<>>(range))
		{
			auto other = cast_<squad_a<>>(range).extract();
