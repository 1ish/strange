TEST_CASE("strange river create_file input code", "[river_t]")
{
	try
	{
		auto river = river_t<>::file(taste_dir + "units/interpretations/parser/strange_taste_code.str");
		REQUIRE(river.good());
	}
...
TEST_CASE("strange tokenize code", "[tokenizer_t]")
{
	try
	{
		auto tokenizer = tokenizer_t<>::create_(river_t<>::file(taste_dir + "units/interpretations/parser/strange_taste_code.str"));
...
TEST_CASE("strange parse and generate code", "[parse_t]")
{
	try
	{
		...
		auto expression = parser.parse_(tokenizer_t<>::create_(river_t<>::file(taste_dir + "units/interpretations/parser/strange_taste_code.str")));
