TEST_CASE("strange river create_file input hello world", "[river_t]")
{
	try
	{
		auto river = river_t<>::file(taste_dir + "units/interpretations/parser/strange_taste_hello_world.str");
		REQUIRE(river.good());
	}
...
TEST_CASE("strange tokenize hello world", "[tokenizer_t]")
{
	try
	{
		auto tokenizer = tokenizer_t<>::create_(river_t<>::file(taste_dir + "units/interpretations/parser/strange_taste_hello_world.str"));
...
TEST_CASE("strange parse and generate hello world", "[parse_t]")
{
	try
	{
		...
		auto expression = parser.parse_(tokenizer_t<>::create_(river_t<>::file(taste_dir + "units/interpretations/parser/strange_taste_hello_world.str")));
